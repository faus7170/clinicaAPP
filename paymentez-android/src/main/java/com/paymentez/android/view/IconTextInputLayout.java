...
public class IconTextInputLayout extends TextInputLayout {
    ...
    private static final String TEXT_FIELD_NAME = "CollapsingTextHelper";
    private static final String RECALCULATE_METHOD_NAME = "recalculate";

    @VisibleForTesting
    Rect mBounds;
    @VisibleForTesting
    Object CollapsingTextHelper;
    @VisibleForTesting
    ...
    void init() {
        try {
            ...
            CollapsingTextHelper = textHeaderField.get(this);

            Field boundsField = CollapsingTextHelper
                    .getClass()
                    .getDeclaredField(BOUNDS_FIELD_NAME);
            boundsField.setAccessible(true);
            mBounds = (Rect) boundsField.get(CollapsingTextHelper);

            mRecalculateMethod = CollapsingTextHelper
                    .getClass()
    ...
    private void adjustBounds() {
        if (CollapsingTextHelper == null || getEditText() == null) {
            return;
        }

        try {
            mBounds.left = getEditText().getLeft() + getEditText().getPaddingLeft();
            mRecalculateMethod.invoke(CollapsingTextHelper);
        }
